...
async function handleIncomingSiprecInvite(req, res, opts) {
  ...
  try {
    ...
    // Stream 2 joins second and is the only leg that starts conference_auto_record
    // (see FSPBX dialplan). Parallel invites caused stream 1 to record ringback alone.
    const uac1 = await srf.createUAC(fsUri, {
      localSdp: sdp1,
    ...
    const uac2 = await srf.createUAC(fsUri, {
      localSdp: sdp2,
      callingNumber: agentExt,
      callingName: agentName,
      calledNumber: 'recorder',
