...
async function handleIncomingSiprecInvite(req, res, opts) {
  ...
  try {
    ...
    callData.streams.add(parsedOpts.sessionId);
    ...
    if (callData.established && callData.finalSdp) {
      opts.logger.info(
      ...
      uas.on('modify', (req, res) => res.send(200, { body: callData.finalSdp }));
      uas.on('destroy', () => releaseDialog(opts, callKey, parsedOpts.sessionId));
      return;
    }
    ...
    // Only stream 1 of a brand-new call may start recording on FSPBX (true_1).
    const stream1IsNewCall = isNewCall;
    ...
    const [uac1, uac2] = await Promise.all([
      srf.createUAC(fsUri, {
        ...
        headers: {
          ...
          'X-Is-New-Call': stream1IsNewCall,
    ...
    uas.on('destroy', () => releaseDialog(opts, callKey, parsedOpts.sessionId));
    uac1.on('destroy', () => teardownFsLegs(opts, callKey));
    uac2.on('destroy', () => teardownFsLegs(opts, callKey));
...
function releaseDialog(opts, callKey, sessionId) {
  const callData = activeCalls.get(callKey);
  if (!callData) return;
  callData.streams.delete(sessionId);
  scheduleTeardown(opts, callKey);
}

function teardownFsLegs(opts, callKey) {
  const callData = activeCalls.get(callKey);
  if (!callData) return;
  callData.uasDialogs.forEach((uas) => { try { uas.destroy(); } catch (e) { /* already gone */ } });
  callData.streams.clear();
  scheduleTeardown(opts, callKey, true);
}

function scheduleTeardown(opts, callKey, forceNow) {
  const callData = activeCalls.get(callKey);
  if (!callData || callData.streams.size > 0) return;
  if (callData.timeout) clearTimeout(callData.timeout);
  const delay = forceNow ? 0 : 10000;
