...
async function handleIncomingSiprecInvite(req, res, opts) {
  ...
  try {
    ...
    let sdp1 = parsedOpts.sdp1.replace(/a=sendonly/g, 'a=sendrecv');
    let sdp2 = parsedOpts.sdp2.replace(/a=sendonly/g, 'a=sendrecv');
    ...
    let mitelSdp = transform.parse(parsedOpts.sdp1);
    let fsSdp1 = transform.parse(uac1.remote.sdp);
    let fsSdp2 = transform.parse(uac2.remote.sdp);
    ...
    let finalSdp = transform.write(mitelSdp);
