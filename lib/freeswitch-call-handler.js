...
// Request-URI uses the FusionPBX domain so calls land in that domain's dialplan.
// INVITE is sent to fsAddress via outbound proxy (not the domain hostname).
const FS_DOMAIN = resolveFsDomain();
const FS_URI    = `sip:${FS_USER}@${FS_DOMAIN}:${FS_PORT}`;
